...
public class Brain {
    ...
    private Brain() {
        ...
        sortTaskMasterList();
        taskObservableList.setAll(
                taskMasterList.stream().filter(task -> !task.isCompleted()).collect(Collectors.toList()));
    ...
    public void initDisplay() {
        ...
        rootController.updateFiniPoints(
                taskMasterList.stream().filter(task -> task.isCompleted()).collect(Collectors.toList()));
    }
    ...
    private String addTask(String commandParameters) {
        ...
        if (!finiParser.getProjectName().equals("Inbox")
                && !projectNameList.contains(finiParser.getProjectName()) && projectNameList.size() == 5) {
        ...
        Task newTask = new Task.TaskBuilder(finiParser.getNotParsed(), finiParser.getIsRecurring())
                .setDatetimes(finiParser.getDatetimes()).setPriority(finiParser.getPriority())
                .setProjectName(finiParser.getProjectName()).setInterval(finiParser.getInterval())
